...
func (s Strategy) String() string {
	...
	return fmt.Sprintf("%s-%dX, Copy: %d, Matched: [%d, %d], PnL: %s, PerH: %.1f%%, PerHLastDay: %.1f%%, PerHLast12Hr: %.1f%%, A: %s%%, D: %.1f%%, 3H: %.1f%%, 2H: %.1f%%, 1H: %.1f%%, MinInv: %s, Grids: %d, Price: %s-%s, %.2f%%",
		...
		s.lastDayRoiChange*100, s.last3HrRoiChange*100, s.last2HrRoiChange*100, s.lastHrRoiChange*100, s.MinInvestment, s.StrategyParams.GridCount, s.StrategyParams.LowerLimit, s.StrategyParams.UpperLimit, s.priceDifference*100)
}
...
func getTopStrategies(strategyType int) (*TrackedStrategies, error) {
	merged, err := mergeStrategies(strategyType,
		...
		SortPair{Sort: SortByRoi, RuntimeMin: 168 * time.Hour, RuntimeMax: 350 * time.Hour},
